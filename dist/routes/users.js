...
const router = Router();
// ============ ADMIN: Get all users ============
router.get('/', verifyAuth, requireRole('bank_admin'), (req, res) => {
    try {
        const users = queryAll('SELECT id, username, role, company_id, created_at FROM users ORDER BY id');
        res.json(users);
    }
    catch (err) {
...
router.post('/', verifyAuth, requireRole('bank_admin'), async (req, res) => {
    try {
        ...
        runSql('INSERT INTO users (username, password_hash, role, company_id, created_at) VALUES (?, ?, ?, ?, ?)', [username, passwordHash, role, role === 'company_user' ? companyId : null, now]);
        saveDb();
        const newUser = queryOne('SELECT id, username, role, company_id, created_at FROM users WHERE username = ?', [username]);
...
// ============ ADMIN: Delete user ============
router.delete('/:id', verifyAuth, requireRole('bank_admin'), (req, res) => {
    try {
        const { id } = req.params;
...
router.put('/me/password', verifyAuth, async (req, res) => {
    try {
...
router.put('/:id/password', verifyAuth, requireRole('bank_admin'), async (req, res) => {
    try {
        const { id } = req.params;
...
router.get('/companies/list', verifyAuth, requireRole('bank_admin'), (req, res) => {
    try {
        const companies = queryAll('SELECT DISTINCT companyId FROM transactions WHERE companyId IS NOT NULL ORDER BY companyId');
        res.json(companies.map((c) => c.companyId));
    }
    catch (err) {
