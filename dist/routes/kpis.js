...
const router = Router();
function calcRecordProfit(record) {
...
router.get('/current', verifyAuth, (req, res) => {
    try {
        ...
        const transferRateResult = queryOne(`SELECT AVG(transfer_exchange_rate) as avg FROM transactions WHERE transfer_exchange_rate IS NOT NULL AND transfer_exchange_rate > 0${companyFilter}`, rateParams);
        const cashRateResult = queryOne(`SELECT AVG(cash_exchange_rate) as avg FROM transactions WHERE cash_exchange_rate IS NOT NULL AND cash_exchange_rate > 0${companyFilter}`, rateParams);
