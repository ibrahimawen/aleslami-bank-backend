 ...
 */
router.get('/activity-heatmap', verifyAuth, (req, res) => {
    try {
        const companyId = req.query.companyId;
        const dateFrom = req.query.dateFrom;
        const dateTo = req.query.dateTo;
        const conditions = ['today IS NOT NULL'];
        const params = [];
        ...
        const data = queryAll(`SELECT today as date, COUNT(*) as count, SUM(COALESCE(final_amount, amount_requested, 0)) as volume
         FROM transactions
         ${whereClause}
         GROUP BY today
         ORDER BY today ASC`, params);
        res.json(data);
    }
    catch (err) {
 ...
 */
router.get('/volume-daily', verifyAuth, (req, res) => {
    try {
        const companyId = req.query.companyId;
        const dateFrom = req.query.dateFrom;
        const dateTo = req.query.dateTo;
        ...
        const whereClause = conditions.length > 0 ? 'WHERE ' + conditions.join(' AND ') : '';
        const data = queryAll(`SELECT today as date,
                COUNT(*) as count,
 ...
 */
router.get('/top-companies', verifyAuth, requireRole('bank_admin'), (req, res) => {
    try {
        const sortBy = req.query.sortBy || 'volume';
        const limit = parseInt(req.query.limit || '10', 10);
        const dateFrom = req.query.dateFrom;
        const dateTo = req.query.dateTo;
        const conditions = ['companyId IS NOT NULL'];
        const params = [];
        if (dateFrom) {
            conditions.push('today >= ?');
            params.push(dateFrom);
        ...
        const data = queryAll(`SELECT companyId,
                COUNT(*) as tx_count,
 ...
 */
router.get('/settlement-summary', verifyAuth, requireRole('bank_admin'), (req, res) => {
    try {
        const totals = queryOne(`SELECT
         ...
         FROM settlements`);
        const recentBatches = queryAll(`SELECT * FROM settlements ORDER BY created_at DESC LIMIT 5`);
        res.json({
            summary: totals || {
