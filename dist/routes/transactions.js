...
router.get('/', verifyAuth, (req, res) => {
    try {
        ...
        LIMIT ? OFFSET ?
      `;
        const records = queryAll(query, [...params, limit, offset]);
...
// ============================================================
router.get('/export/csv', verifyAuth, (req, res) => {
    try {
        const { clause, params } = buildWhereClause(req.user, {
            companyId: req.query.companyId,
            typeCode: req.query.typeCode,
...
// ============================================================
router.get('/export/excel', verifyAuth, (req, res) => {
    try {
        const { clause, params } = buildWhereClause(req.user, {
            companyId: req.query.companyId,
            typeCode: req.query.typeCode,
        ...
        const records = queryAll(query, filterParams);
...
router.get('/:reference', verifyAuth, (req, res) => {
    try {
        const { reference } = req.params;
        const query = 'SELECT * FROM transactions WHERE reference = ?';
        const transaction = queryOne(query, [reference]);
        if (!transaction) {
