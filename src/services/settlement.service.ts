 ...
 */
function determineChannel(tx: TransactionRow): ExecutionChannel {
 ...
 */
function getActualRate(tx: TransactionRow, channel: ExecutionChannel): number {
 ...
 */
function isTransactionCompleted(tx: TransactionRow): boolean {
  return (
 ...
 */
function validateEligibility(tx: TransactionRow): { isEligible: boolean; errors: string[] } {
  const errors: string[] = [];
 ...
 */
function determineSettlementAmounts(tx: TransactionRow): SettlementAmounts {
 ...
 */
function determineSettlementDecision(
  tx: TransactionRow,
  actualRate: number,
  ...
  const flags: string[] = [];
  if (!tx.approved && !tx.processed) flags.push('Not approved or processed');
  if (!actualRate || actualRate === 0) flags.push('Missing or zero exchange rate');
 ...
 */
function analyzeTransaction(tx: TransactionRow, purchaseRate: number, useHistorical: boolean): SettlementResult {
  ...
  let status: SettlementStatus = 'Rejected from Settlement';
  let decision_reason = 'Transaction does not meet settlement criteria';
  let review_flags: string[] = [];
  let channel: ExecutionChannel = 'unknown';
  let actualRate = 0;
  let amountUSD = 0;
  let amountLYD = 0;
  let grossMargin = 0;
 ...
 */
export function runSettlementAnalysis(params: AnalyzeParams): SettlementAnalyzeResponse {
  ...
  const allResults = transactions.map(tx =>
    analyzeTransaction(tx, params.purchaseRate, params.useHistorical)
  );

  // الربح يحتسب فقط على حوالات غير مرفوضة
  const profitableResults = allResults.filter(r =>
    r.settlement_status !== 'Rejected from Settlement' && r.execution_channel !== 'cash'
  );
 ...
 */
export function getSettlementExportData(params: AnalyzeParams): SettlementResult[] {
  const transactions = findTransactions({
    companyId: params.companyId,
    dateFrom: params.dateFrom,
    dateTo: params.dateTo,
