 ...
 */
function getHistoricalRate(txDate) {
    try {
        const rate = queryOne('SELECT rate FROM purchase_rates WHERE effective_date <= ? ORDER BY effective_date DESC LIMIT 1', [txDate]);
 ...
 */
function determineSettlementAmounts(tx, channel) {
 ...
 */
function determineSettlementStatus(tx, channel, actualRate, settlementAmount, errors) {
    const review_flags = [];
 ...
 */
function analyzeTransaction(tx, purchaseRate = 7.37, useHistorical = false) {
    ...
    if (isEligible) {
        ...
        const decision = determineSettlementStatus(tx, execution_channel, actual_rate, settlementData, []);
 ...
 */
router.get('/analyze', verifyAuth, requireRole('bank_admin'), (req, res) => {
    try {
        ...
        const transactions = queryAll(sqlQuery, queryParams);
        ...
        const results = transactions.map(tx => analyzeTransaction(tx, purchaseRate, useHistorical));
        ...
        const profitableResults = results.filter(r => r.settlement_status !== 'Rejected from Settlement' &&
 ...
 */
router.get('/export', verifyAuth, requireRole('bank_admin'), (req, res) => {
    try {
        ...
        // Query transactions
        const sqlQuery = `SELECT * FROM transactions ${whereClause} ORDER BY timestamp DESC`;
        const transactions = queryAll(sqlQuery, exportParams);
        // Determine if we should use historical rates
        const userProvidedRate = purchaseRateStr !== undefined && purchaseRateStr !== '7.37';
        const useHistorical = !userProvidedRate;
