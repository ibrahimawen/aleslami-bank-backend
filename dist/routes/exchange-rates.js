...
import { upsertDailyRates, getRateHistory, getRateSetForDate, getAllRates, validateRate, } from '../services/rate-service.js';
const router = Router();
/**
 * POST /api/rates/daily - Create or update daily rate set (bank_admin only)
 */
router.post('/daily', verifyAuth, requireRole('bank_admin'), (req, res) => {
    try {
        ...
        for (const [key, value] of Object.entries(optionalRates)) {
            if (value !== undefined && value !== null && !validateRate(value)) {
