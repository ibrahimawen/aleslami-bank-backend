 ...
 */
export function getRateSetForDate(date) {
    const rate = queryOne(`SELECT * FROM exchange_rates_daily
     WHERE effective_date <= ?
     ORDER BY effective_date DESC LIMIT 1`, [date]);
    return rate;
}
/**
 * Get the applicable rate based on deposit type and date
 */
export function getApplicableRate(depositType, date) {
    const rateSet = getRateSetForDate(date);
    if (!rateSet)
        return DEFAULT_CENTRAL_RATE;
    switch (depositType) {
        case 'transfer':
 ...
 */
export function getCentralBankRate(date) {
    const rateSet = getRateSetForDate(date);
 ...
 */
export function getSellRate(date) {
    const rateSet = getRateSetForDate(date);
 ...
 */
export function calculateProfit(usdAmount, sellRate, centralBankRate) {
    if (!usdAmount || usdAmount <= 0 || !sellRate || sellRate <= 0)
        return 0;
 ...
 */
export function upsertDailyRates(data, userId) {
    const now = new Date().toISOString();
    // Check if rate exists for this date
    const existing = queryOne('SELECT * FROM exchange_rates_daily WHERE effective_date = ?', [data.effective_date]);
    if (existing) {
        // Update
        runSql(`UPDATE exchange_rates_daily SET
        central_bank_rate = ?,
        sell_rate = ?,
        transfer_rate = ?,
        cash_rate = ?,
      ...
      WHERE effective_date = ?`, [
        ...
        // Insert
        runSql(`INSERT INTO exchange_rates_daily
    ...
    runSql(`INSERT OR REPLACE INTO purchase_rates (rate, effective_date, notes, created_by, created_at)
     VALUES (?, ?, ?, ?, ?)`, [data.central_bank_rate, data.effective_date, data.notes || null, userId, now]);
    saveDb();
    return queryOne('SELECT * FROM exchange_rates_daily WHERE effective_date = ?', [data.effective_date]);
}
/**
...
export function getRateHistory(options) {
    ...
    const countResult = queryOne(`SELECT COUNT(*) as count FROM exchange_rates_daily ${whereClause}`, params);
    const rates = queryAll(`SELECT * FROM exchange_rates_daily ${whereClause} ORDER BY effective_date DESC LIMIT ? OFFSET ?`, [...params, limit, offset]);
    return {
        rates,
 ...
 */
export function getAllRates(dateFrom, dateTo) {
    const conditions = [];
    const params = [];
    if (dateFrom) {
        conditions.push('effective_date >= ?');
    ...
    const whereClause = conditions.length > 0 ? 'WHERE ' + conditions.join(' AND ') : '';
    return queryAll(`SELECT * FROM exchange_rates_daily ${whereClause} ORDER BY effective_date DESC`, params);
}
