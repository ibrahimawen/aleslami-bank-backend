...
router.post(
  ...
  (req: Request<{}, {}, CreateRateRequest>, res: Response): void => {
    ...
    try {
      ...
      runSql(
        `INSERT OR REPLACE INTO purchase_rates (rate, effective_date, notes, created_by, created_at)
         VALUES (?, ?, ?, ?, ?)`,
        [rate, effective_date, notes || null, userId || null, createdAt]
      );
      saveDb();

      const savedRate = queryOne(
        'SELECT * FROM purchase_rates WHERE effective_date = ?',
        [effective_date]
      );
...
router.get('/current', verifyAuth, (req: Request, res: Response): void => {
  try {
    const rate = queryOne(
      'SELECT * FROM purchase_rates ORDER BY effective_date DESC LIMIT 1'
    );
...
router.get(
  '/history',
  verifyAuth,
  (req: Request<{}, {}, {}, RateHistoryQuery>, res: Response): void => {
    try {
...
router.get(
  '/for-date',
  verifyAuth,
  (req: Request<{}, {}, {}, RateForDateQuery>, res: Response): void => {
    try {
...
router.get(
  '/export',
  verifyAuth,
  (req: Request<{}, {}, {}, ExportQuery>, res: Response): void => {
    try {
      const dateFrom = req.query.dateFrom || null;
      const dateTo = req.query.dateTo || null;
      ...
      const rates = queryAll(
        `SELECT * FROM purchase_rates ${whereClause} ORDER BY effective_date DESC`,
        params
      );
