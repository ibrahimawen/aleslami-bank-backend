 ...
 */
function getComputedStatus(record) {
    ...
    if (rejected === 1)
        return 'rejected';
    if (error1 && error1.toLowerCase().includes('insufficient'))
        return 'insufficient';
    if (error2 && error2.trim() !== '')
        return 't24_error';
    if (approved === 1 && processed === 1)
        return 'completed';
    if (approved === 1 && processed === 0)
        return 'approved';
    if (charge === 1)
        return 'charged';
    return 'pending';
 ...
 */
function parseInt0(val, defaultVal = 0) {
    if (val === null || val === undefined || val === '')
        return defaultVal;
...
async function seedUsers() {
    ...
    if (!existingAdmin) {
        ...
        runSql(`INSERT INTO users (username, password_hash, role, company_id, created_at) VALUES (?, ?, ?, ?, ?)`, ['admin', adminHash, 'bank_admin', null, now]);
    ...
    if (!existingCompany1) {
        ...
        runSql(`INSERT INTO users (username, password_hash, role, company_id, created_at) VALUES (?, ?, ?, ?, ?)`, ['company1', company1Hash, 'company_user', 'LY0010002', now]);
    ...
    if (!existingCompany2) {
        ...
        runSql(`INSERT INTO users (username, password_hash, role, company_id, created_at) VALUES (?, ?, ?, ?, ?)`, ['company2', company2Hash, 'company_user', 'LY0010017', now]);
    ...
    if (!existingCompany) {
        ...
        runSql(`INSERT INTO users (username, password_hash, role, company_id, created_at) VALUES (?, ?, ?, ?, ?)`, ['company', companyHash, 'company_user', null, now]);
...
async function seedDatabase() {
    ...
    for (const record of transactions) {
        ...
        try {
            runSql(`INSERT OR IGNORE INTO transactions (
