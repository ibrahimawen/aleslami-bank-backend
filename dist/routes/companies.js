...
router.get('/', verifyAuth, requireRole('bank_admin'), (req, res) => {
    try {
        ...
        ORDER BY companyId
      `;
        const companies = queryAll(companiesQuery);
        const stats = [];
        for (const { companyId } of companies) {
            const recordsQuery = `
          SELECT * FROM transactions WHERE companyId = ?
        `;
            const records = queryAll(recordsQuery, [companyId]);
            ...
            for (const r of records) {
                ...
                if (r.deposit_type === 'transfer')
                    contractMap[cid].transferCount++;
                if (r.deposit_type === 'cash')
                    contractMap[cid].cashCount++;
...
router.get('/:id', verifyAuth, requireRole('bank_admin'), (req, res) => {
    try {
        const { id } = req.params;
        const recordsQuery = `
        SELECT * FROM transactions WHERE companyId = ? ORDER BY timestamp DESC
      `;
        const records = queryAll(recordsQuery, [id]);
        if (records.length === 0) {
